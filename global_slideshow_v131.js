/**
2010-09-03: for Moz, changed MozOpacity -> just opacity
2009-09-04: search initialBlockSet in fifo() - fixed bug of both ly1 and ly2 showing initially
2009-08-10: jumped to v1.30 to keep up with component
2007-01-30: reviewed
2005-09-19:
-----------
v101 differs from v100 in that the head vars have been moved here (exc. for the dynamically created ones)
**/
var initialBlockSet=false;
...
if(typeof ssJsSetHeight=='undefined')ssJsSetHeight=true;
...
function toggle_description(){
	if(g('ssDescription').style.display=='block'){
		...
		g('ssDescription').style.display='block'
		g('pictureTextText').style.display='none';
		//transfer text over
		g('ssDescription').innerHTML=g('Description').value;
	}
}
...
function previousNext(s,absolute){
	...
	if(ssJsSetHeight)g('img1').height=fheight[idxTo];

	if(!editing){
		/* blank alternative: 'Untitled Slide' */
		if(ssUseDynamicTitles)g('ssTitle').innerHTML=(g('title_'+idxTo).innerHTML ? g('title_'+idxTo).innerHTML : (useUntitledTitle ? 'Untitled '+pictureWord : '&nbsp;'));
		...
		if(ssUseDynamicDescriptions)g('ssDescription').innerHTML=(g('Description').value ? g('Description').value : (g('description_'+idxTo).getAttribute('filename') ? g('description_'+idxTo).getAttribute('filename') : '&nbsp;' ));
	...
	if(browser=='IE'){
		...
		g('ly1').style.opacity=1.0;
		g('ly2').style.opacity=0.0;
	}
	idx=idxTo;
	idxNext=idxToNext;
	g('img2').src=imgs[idxToNext].src;
	g('img2').width=fwidth[idxToNext];
	if(ssJsSetHeight)g('img2').height=fheight[idxToNext];
	gallery_highlight(idx);
	return false;
}
function fifo(){
	if(!initialBlockSet){
		g('ly2').style.display='block';
		if(browser=='IE'){
			g('ly2').style.filter='alpha(opacity=0)';
		}else{
			g('ly2').style.opacity=0;
		}
	...
	if(browser=='IE'){
		...
		var f1=parseInt(g('ly1').style.opacity*100);
		var f2=parseInt(g('ly2').style.opacity*100);
	}
	if(f1>0){
	...
	if(f1>0 || f2<100){
		...
		if(ssJsSetHeight)g('img1').height=fheight[idxNext];
		//insert text and title for this picture
		if(!editing){
			/* blank alternative: 'Untitled Slide' */
			if(ssUseDynamicTitles)g('ssTitle').innerHTML=(g('title_'+idxNext).innerHTML ? g('title_'+idxNext).innerHTML : (useUntitledTitle ? 'Untitled '+pictureWord : '&nbsp;'));
			...
			if(ssUseDynamicDescriptions)g('ssDescription').innerHTML=(g('Description').value ? g('Description').value : (g('description_'+idxNext).getAttribute('filename') ? g('description_'+idxNext).getAttribute('filename') : '&nbsp;' ));
		...
		if(typeof galleryFrameCount!=='undefined'){
			if(galleryFrameCount>1)gallery_frameswap(idx,'idx');
			gallery_highlight(idx);
		}
		...
		setTimeout("if(ssJsSetHeight)g('img2').height=fheight[idxNext];",500)
...
function gallery_highlight(idx){
	for(var i=1; i<=imgcount; i++){
		try{ g('gallerythumb_'+i).className=(i==idx?'galleryThumb on':'galleryThumb');}
		catch(e){ }
	}
}
