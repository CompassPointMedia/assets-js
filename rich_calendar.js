...
// show calendar
var show_cal_text_field=null;
function show_cal(el,t) {
	if (cal_obj) return;
	var text_field = document.getElementById(t);
	show_cal_text_field=t;
...
function cal_on_change(cal, object_code) {
	if (object_code == 'day') {
		if(document.getElementById(show_cal_text_field).value!==cal.get_formatted_date(format)){
			mgeChge(document.getElementById(show_cal_text_field));
		}
		document.getElementById(show_cal_text_field).value = cal.get_formatted_date(format);
		cal.hide();
		cal_obj = null;
	}
}
...
function cal_on_close(cal) {
	if (true || window.confirm('Are you sure to close the calendar?')) {
		cal.hide();
		cal_obj = null;
	}
}
