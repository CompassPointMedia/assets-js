...
function h(x, grp, multi, evt, event){
	...
	try{document.selection.empty();} catch(e){
		if(e.description && false){ alert('File: dataobjects_i1_v200.js\nLine: 26\n'+e.description);}
	}
	var eck=false;
...
function datasetSort2(component,col,e,p,s){
	//2012-12-21 added parameter s as a straight string to add to query string - soon this will be jquery
	if(typeof e== 'undefined')e=window.event;
	if(typeof p!== 'undefined'){
		...
		for(var i in param){
			if(param[i].indexOf(':')!==-1){
				...
				try{ str+=param[i]+'='+escape(g(param[i]).value)+'&'; }catch(e){ }
			}
		}
		str=str.replace(/&$/,'');
	}
	...
	if(e.shiftKey)sortShift=1;
	if(e.altKey)sortAlt=1;
	window.open('resources/bais_01_exe.php?mode=refreshComponent&component='+component+'&sort='+col+'&sortAlt='+sortAlt+'&sortCtrl='+sortCtrl+'&sortShift='+sortShift+(str?'&'+str:'')+(typeof s!='undefined'?'&'+s:''),'w2');
	return false;
}